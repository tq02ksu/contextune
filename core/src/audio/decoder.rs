...
use crate::audio::ring_buffer::{
    AudioRingBuffer, RingBufferConfig, RingBufferConsumer, RingBufferProducer,
};
use crate::Result;
use std::fs::File;
use std::path::Path;
...
impl AudioDecoder {
    ...
    pub fn duration_seconds(&self) -> Option<f64> {
        self.duration
            .map(|frames| frames as f64 / self.format.sample_rate as f64)
    }
        ...
        match buffer {
            ...
            AudioBufferRef::U24(buf) => {
                for frame in 0..buf.frames() {
                    for ch in 0..buf.spec().channels.count() {
                        let sample =
                            buf.chan(ch)[frame].inner() as f64 / (1u32 << 24) as f64 * 2.0 - 1.0;
...
impl AudioStreamReader {
    ...
    pub fn next_packet(&mut self) -> Result<Option<DecodedPacket>> {
        match self.packet_receiver.try_recv() {
            ...
            Err(mpsc::TryRecvError::Disconnected) => Err(crate::Error::Decoding(
                "Decoder thread disconnected".to_string(),
            )),
        }
    }
    ...
    pub fn next_packet_blocking(&mut self) -> Result<Option<DecodedPacket>> {
        match self.packet_receiver.recv() {
            Ok(result) => result,
            Err(mpsc::RecvError) => Err(crate::Error::Decoding(
                "Decoder thread disconnected".to_string(),
            )),
        }
    }
...
impl AudioStreamReaderWithRingBuffer {
    ...
    pub fn new<P: AsRef<Path>>(
        path: P,
        config: StreamConfig,
    ) -> Result<(Self, RingBufferConsumer)> {
        let decoder = Arc::new(Mutex::new(AudioDecoder::new(&path)?));
        let stop_flag = Arc::new(Mutex::new(false));
        ...
        let ring_buffer_config = RingBufferConfig {
            buffer_duration_seconds: config.buffer_size as f64 / audio_format.sample_rate as f64
                * 4.0, // 4x buffer size
            format: audio_format,
        ...
        let (producer, consumer) = AudioRingBuffer::new(ring_buffer_config).map_err(|e| {
            crate::Error::AudioEngine(format!("Failed to create ring buffer: {}", e))
        })?;
...
mod tests {
    ...
    fn test_decoder_creation_for_each_format() {
        ...
        for format in formats {
            ...
            assert!(
                result.is_err(),
                "Decoder should fail for non-existent {}",
                format
            );
            ...
            assert!(
                result.is_err(),
                "Decoder should fail for invalid {} file",
                format
            );
    ...
    fn test_stream_reader_creation_for_each_format() {
        ...
        for format in formats {
            ...
            assert!(
                result.is_err(),
                "Stream reader should fail for non-existent {}",
                format
            );
            ...
            assert!(
                result.is_err(),
                "Looping stream reader should fail for non-existent {}",
                format
            );
            ...
            assert!(
                result.is_err(),
                "Custom stream reader should fail for non-existent {}",
                format
            );
        }
    }
