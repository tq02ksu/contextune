...
mod tests {
    ...
    fn test_white_noise_flat_spectrum() {
        ...
        let (samples, sample_rate) = read_wav_samples(test_file)
            .expect("Should read white noise");
        
        let spectrum = analyze_frequency_content(&samples, sample_rate);
        
        // Calculate average magnitude in different frequency bands
        let low_band: f64 = spectrum.iter()
            .filter(|(f, _)| *f >= 100.0 && *f < 1000.0)
            .map(|(_, mag)| mag)
            .sum::<f64>() / spectrum.iter().filter(|(f, _)| *f >= 100.0 && *f < 1000.0).count() as f64;
        
        let mid_band: f64 = spectrum.iter()
            .filter(|(f, _)| *f >= 1000.0 && *f < 5000.0)
            .map(|(_, mag)| mag)
            .sum::<f64>() / spectrum.iter().filter(|(f, _)| *f >= 1000.0 && *f < 5000.0).count() as f64;
        
        let high_band: f64 = spectrum.iter()
            .filter(|(f, _)| *f >= 5000.0 && *f < 10000.0)
            .map(|(_, mag)| mag)
            .sum::<f64>() / spectrum.iter().filter(|(f, _)| *f >= 5000.0 && *f < 10000.0).count() as f64;
    ...
    fn test_silence_no_frequency_content() {
        ...
        let max_magnitude = spectrum.iter()
            .map(|(_, mag)| mag)
    ...
    fn test_impulse_wide_spectrum() {
        ...
        let (samples, sample_rate) = read_wav_samples(test_file)
            .expect("Should read impulse");
        
        let spectrum = analyze_frequency_content(&samples, sample_rate);
        
        // Impulse should have relatively flat spectrum (like white noise)
        // Check that energy is present across frequency range
        let low_energy: f64 = spectrum.iter()
            .filter(|(f, _)| *f >= 100.0 && *f < 1000.0)
            .map(|(_, mag)| mag * mag)
            .sum();
        
        let high_energy: f64 = spectrum.iter()
            .filter(|(f, _)| *f >= 5000.0 && *f < 10000.0)
            .map(|(_, mag)| mag * mag)
            .sum();
        
        // Both low and high frequencies should have significant energy
        assert!(low_energy > 0.01, "Impulse should have low frequency energy");
        assert!(high_energy > 0.01, "Impulse should have high frequency energy");
    }
    ...
    fn test_frequency_sweep_coverage() {
        ...
        let (samples, sample_rate) = read_wav_samples(test_file)
            .expect("Should read frequency sweep");
