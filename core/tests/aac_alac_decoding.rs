...
use contextune_core::audio::decoder::{detect_format, is_format_supported, AudioDecoder};
use std::io::Write;
...
mod tests {
    ...
    fn test_format_detection_with_aac_content() {
        ...
        assert!(
            result.is_err(),
            "Should fail to detect format for invalid AAC content"
        );
    }
    ...
    fn test_format_detection_with_m4a_content() {
        ...
        assert!(
            result.is_err(),
            "Should fail to detect format for invalid M4A content"
        );
    }
    ...
    fn test_supported_extensions_include_aac_formats() {
        ...
        assert!(
            extensions.contains(&"aac"),
            "AAC should be in supported extensions"
        );
        assert!(
            extensions.contains(&"m4a"),
            "M4A should be in supported extensions"
        );
    }
    ...
    fn test_alac_format_info_properties() {
        ...
        assert!(
            alac_info.is_high_resolution(),
            "ALAC with 96kHz/24bit should be hi-res"
        );
    }
    ...
    fn test_high_resolution_aac_support() {
        ...
        assert!(
            hires_aac_info.is_high_resolution(),
            "48kHz AAC should be considered hi-res"
        );
        assert!(
            !hires_aac_info.is_lossless,
            "AAC should still be lossy even at hi-res"
        );
    }
    ...
    fn test_high_resolution_alac_support() {
        ...
        assert!(
            hires_alac_info.is_high_resolution(),
            "192kHz/24bit ALAC should be hi-res"
        );
