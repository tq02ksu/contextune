...
fn test_closure_memory_leak() {
    ...
    for i in 0..1000 {
        ...
        let processor = |multiplier: f32| {
            data.iter().map(|&x| x * multiplier).collect::<Vec<f32>>()
        };
        ...
        // Combine results
        let combined: Vec<f32> = result1.iter().zip(result2.iter())
            .map(|(&a, &b)| a + b)
            .collect();
...
fn test_memory_stress() {
    ...
    for iteration in 0..ITERATIONS {
        ...
        let nested: Vec<Vec<u8>> = (0..10).map(|i| {
            (0..100).map(|j| (i + j) as u8).collect()
        }).collect();
