...
mod null_pointer_tests {
    ...
    fn test_null_handle_operations() {
        unsafe {
            ...
            assert_eq!(audio_engine_get_position(null_handle, &mut position), FFIResult::NullPointer);
        }
    }
...
mod concurrent_access_tests {
    ...
    fn test_concurrent_playback_control() {
        unsafe {
            ...
            let threads: Vec<_> = (0..NUM_OPERATIONS).map(|i| {
                let barrier = Arc::clone(&barrier);
                thread::spawn(move || {
                    barrier.wait();
                    unsafe {
...
mod type_conversion_tests {
    ...
    use contexture_core::ffi::types::{
        rust_string_to_c_string, c_string_to_rust_str, validate_not_null, validate_not_null_mut
    };
    ...
    fn test_volume_range_validation() {
        unsafe {
            ...
            assert_eq!(audio_engine_set_volume(handle3, -0.001), FFIResult::InvalidArgument);
            
            let handle4 = audio_engine_create();
            assert_eq!(audio_engine_set_volume(handle4, 1.001), FFIResult::InvalidArgument);
            
            let handle5 = audio_engine_create();
            assert_eq!(audio_engine_set_volume(handle5, f64::NAN), FFIResult::InvalidArgument);
            
            let handle6 = audio_engine_create();
            assert_eq!(audio_engine_set_volume(handle6, f64::INFINITY), FFIResult::InvalidArgument);
            
            let handle7 = audio_engine_create();
            assert_eq!(audio_engine_set_volume(handle7, f64::NEG_INFINITY), FFIResult::InvalidArgument);
    ...
    fn test_rapid_operations() {
        unsafe {
            ...
            for i in 0..100 {
                ...
                let mut position = 0.0;
                assert_eq!(audio_engine_get_position(handle, &mut position), FFIResult::Success);
