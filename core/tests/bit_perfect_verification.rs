...
mod tests {
    ...
    fn test_read_reference_sine_wave() {
        ...
        assert_eq!(
            samples.len(),
            44100 * 5,
            "Should have 5 seconds of audio at 44.1kHz"
        );
    }
    ...
    fn test_silence_is_zero() {
        ...
        assert_eq!(
            non_zero_count, 0,
            "Silence should contain only zero samples"
        );
    }
    ...
    fn test_impulse_has_single_peak() {
        ...
        assert!(
            samples[0].abs() > 30000,
            "First sample should be near maximum"
        );
    ...
    fn test_sine_wave_properties() {
        ...
        let sum_squares: f64 = samples.iter().map(|&s| (s as f64 / 32767.0).powi(2)).sum();
    ...
    fn test_white_noise_properties() {
        ...
        let mean: f64 =
            samples.iter().map(|&s| s as f64 / 32767.0).sum::<f64>() / samples.len() as f64;
